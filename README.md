...
- **宿舍区**(`SZU_CTC&CMCC`):Dr.COM 网页认证,断网后自动重新登录
...
**第 1 条:把项目下载到你的电脑**(几秒完成)

```bash
git clone https://github.com/JennieYow/szu-net-autologin.git
```

**第 2 条:进入项目文件夹**(必须先进文件夹,否则后面会报 `no such file or directory`)

```bash
cd szu-net-autologin
```

**第 3 条:给安装脚本开执行权限**(只需一次)

```bash
chmod +x install.sh
```

**第 4 条:运行安装程序**(它会停下来问你问题)
