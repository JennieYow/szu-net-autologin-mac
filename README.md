...
> 一句话原理:一个小脚本每 45 秒检查一次网络,发现掉线就替你向认证服务器"点一次登录按钮"——账号密码只存在你本机的钥匙串里。
...
<details>
<summary>教学区登录一直失败:"无返回信息"?</summary>

学校可能调整了认证接口参数。浏览器 F12 打开网络面板,手动登录一次,核对 `srun_portal` 请求中的 `ac_id` 等参数,修改 `config.sh` 中的 `SRUN_AC_ID`。
</details>
